        ...
        <el-card class="box-card" style="width:80%;margin:20px auto">
          <el-form ref="spuBaseForm" :model="spu" label-width="120px" :rules="spuBaseInfoRules">
            ...
            <el-form-item label="设置积分" prop="bounds">
              <label>金币</label>
              <el-input-number v-model="spu.bounds.buyBounds" style="width:130px" placeholder="金币" :min="0" controls-position="right" />
              <label style="margin-left:15px">成长值</label>
              <el-input-number v-model="spu.bounds.growBounds" style="width:130px" placeholder="成长值" :min="0" controls-position="right">
                <template slot="prepend">成长值</template>
              </el-input-number>
            </el-form-item>
            <el-form-item label="商品介绍" prop="decript">
              <multi-upload ref="decript" v-model="spu.decript" />
            </el-form-item>

            <el-form-item label="商品图集" prop="images">
              <multi-upload ref="images" v-model="spu.images" />
            </el-form-item>
            <el-form-item>
      ...
      <el-col v-show="step==1" :span="24">
        <el-card class="box-card" style="width:80%;margin:20px auto">
          <el-tabs tab-position="left" style="width:98%">
            <el-tab-pane v-for="(group,gidx) in dataResp.attrGroups" :key="group.attrGroupId" :label="group.attrGroupName">
              <!-- 遍历属性,每个tab-pane对应一个表单，每个属性是一个表单项  spu.baseAttrs[0] = [{attrId:xx,val:}]-->
              <el-form ref="form" :model="spu">
                <el-form-item v-for="(attr,aidx) in group.attrs" :key="attr.attrId" :label="attr.attrName">
                  <el-input v-show="false" v-model="dataResp.baseAttrs[gidx][aidx].attrId" type="hidden" />
          ...
          <el-card class="box-card">
            <div slot="header" class="clearfix">
              ...
              <el-form ref="saleform" :model="spu">
                <el-form-item v-for="(attr,aidx) in dataResp.saleAttrs" :key="attr.attrId" :label="attr.attrName">
                  <el-input v-show="false" v-model="dataResp.tempSaleAttrs[aidx].attrId" type="hidden" />
                  <el-checkbox-group v-model="dataResp.tempSaleAttrs[aidx].attrValues">
                    <template v-if="dataResp.saleAttrs[aidx].valueSelect !== ''">
                      <el-checkbox v-for="val in dataResp.saleAttrs[aidx].valueSelect.split(';')" :key="val" :label="val" />
                    </template>
        ...
        <el-card class="box-card" style="width:80%;margin:20px auto">
          <el-table :data="spu.skus" style="width: 100%">
            <el-table-column label="属性组合">
              <el-table-column v-for="(item,index) in dataResp.tableAttrColumn" :key="item.attrId" :label="item.attrName">
              ...
              <template slot-scope="scope">
                <el-row>
                  <el-col :span="24">
                    ...
                    <multi-upload v-model="uploadImages" style="float:left;margin-left:10px;" :show-file="false" :list-type="'text'" />
                  </el-col>
                  <el-col :span="24">
                    <el-divider />
                  </el-col>
                  <el-col :span="24">
                    <el-card v-for="(img,index) in spu.images" :key="index" style="width:170px;float:left;margin-left:15px;margin-top:15px;" :body-style="{ padding: '0px' }">
                  ...
                  <el-row>
                    <el-col :span="24">
                      <el-form-item label="设置折扣">
                        ...
                        <el-input-number v-model="scope.row.discount" style="width:160px" :precision="2" :max="1" :min="0" :step="0.01" controls-position="right" />
                        <label>折</label>
...
import ShopSelect from '../common/shop-select'
import MultiUpload from '@/components/upload/multiUpload'
export default {
  // import引入的组件需要注入到对象中才能使用
  components: { CategoryCascader, BrandSelect, MultiUpload, ShopSelect },
  props: {},
  data() {
    return {
      loading: false,
      ...
      spuBaseInfoRules: {
        ...
        shopId: [
          { required: true, message: '请选择一个店铺', trigger: 'blur' }
        ],
        catalogId: [
  ...
  watch: {
    uploadImages(val) {
      ...
      this.spu.skus.forEach((item, index) => {
        const len = imgArr.length - this.spu.skus[index].images.length // 还差这么多
        if (len > 0) {
  ...
  mounted() {
    ...
    // eslint-disable-next-line no-undef
    this.shopIdSub = PubSub.subscribe('shopId', (msg, val) => {
      this.spu.shopId = val
  ...
  beforeDestroy() {
    ...
    // eslint-disable-next-line no-undef
    PubSub.unsubscribe(this.shopIdSub)
  },
  destroyed() {},
  activated() {},
  // 方法集合
  methods: {
    ...
    resetSpuData() {
      ...
      this.$refs.decript.fileList = []
      this.$refs.images.fileList = []
      this.$refs.decript.fileList2 = []
      this.$refs.images.fileList2 = []
    },
    ...
    generateSkus() {
      ...
      const descartes = this.descartes(selectValues)
