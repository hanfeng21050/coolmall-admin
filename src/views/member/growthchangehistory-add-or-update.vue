  ...
  <el-dialog :title="!dataForm.id ? '新增' : '修改'" :close-on-click-modal="false" :visible.sync="visible">
    <el-form ref="dataForm" :model="dataForm" :rules="dataRule" label-width="80px" @keyup.enter.native="dataFormSubmit()">
      ...
      <el-form-item label="改变的值（正负计数）" prop="changeCount">
        <el-input v-model="dataForm.changeCount" placeholder="改变的值（正负计数）" />
      </el-form-item>
      <el-form-item label="备注" prop="note">
        <el-input v-model="dataForm.note" placeholder="备注" />
      </el-form-item>
      <el-form-item label="积分来源[0-购物，1-管理员修改]" prop="sourceType">
        <el-input v-model="dataForm.sourceType" placeholder="积分来源[0-购物，1-管理员修改]" />
      </el-form-item>
    </el-form>
    ...
    return {
      ...
      dataRule: {
        ...
        sourceType: [
          {
            required: true,
            message: '积分来源[0-购物，1-管理员修改]不能为空',
            trigger: 'blur'
          }
        ]
      }
        ...
        if (this.dataForm.id) {
          this.$http({
            url: this.$http.adornUrl(
              `/member/growthchangehistory/info/${this.dataForm.id}`
            ),
            method: 'get',
